...
public final class ActivityThread extends ClientTransactionHandler {
    ...
    private class ApplicationThread extends IApplicationThread.Stub {
        ...
        @Override
        public void requestRuntimeView(IBinder activityToken) {
            ActivityClientRecord acr = mActivities.get(activityToken);
            RuntimeView rv = new RuntimeView(acr.activity.getWindow().getDecorView().getRootView());
            try {
                IBinder b = ServiceManager.getService("runtime_monitor");
                IRuntimeMonitor rm = IRuntimeMonitor.Stub.asInterface(b);
                rm.reportRuntimeView(rv, Process.myPid());
            } catch (Exception e) {
        ...
        @Override
        public void requestHeapInfo() {
            final Runtime r = Runtime.getRuntime();
            try {
                IBinder b = ServiceManager.getService("runtime_monitor");
                IRuntimeMonitor rm = IRuntimeMonitor.Stub.asInterface(b);
                rm.reportHeapInfo(r.maxMemory() / 1024, r.totalMemory() / 1024, r.freeMemory() / 1024, Process.myPid());
            } catch (Exception e) {
                Slog.i(TAG, e.getMessage());
            }
        }
