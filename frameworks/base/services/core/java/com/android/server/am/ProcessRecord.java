...
public final class ProcessRecord {
    ...
    final ArraySet<BroadcastRecord> curReceivers = new ArraySet<BroadcastRecord>();// receivers currently running in the
                                                                                   // app
    ...
    // These reports are generated & stored when an app gets into an error
    // condition.
    ...
    // App is allowed to manage whitelists such as temporary Power Save mode
    // whitelist.
    ...
    String seInfo;
    long startTime;
    // This will be same as {@link #uid} usually except for some apps used during
    // factory testing.
    int startUid;
    ...
    void dump(PrintWriter pw, String prefix) {
        ...
        if (hasTopUi || hasOverlayUi || runningRemoteAnimation) {
            ...
            pw.println(runningRemoteAnimation);
        }
    ...
    ProcessRecord(ActivityManagerService _service, BatteryStatsImpl _batteryStats, ApplicationInfo _info,
    ...
    public void makeActive(IApplicationThread _thread, ProcessStatsService tracker) {
        if (thread == null) {
            ...
            if (origBase != null) {
                origBase.setState(ProcessStats.STATE_NOTHING, tracker.getMemFactorLocked(), SystemClock.uptimeMillis(),
                        pkgList);
                origBase.makeInactive();
            }
            baseProcessTracker = tracker.getProcessStateLocked(info.packageName, uid, info.longVersionCode,
                    processName);
            ...
            for (int i = 0; i < pkgList.size(); i++) {
                ...
                holder.state = tracker.getProcessStateLocked(pkgList.keyAt(i), uid, info.longVersionCode, processName);
    ...
    /**
     * This method returns true if any of the activities within the process record
     * are interesting to the user. See HistoryRecord.isInterestingToUserLocked()
     */
    ...
    void scheduleCrash(String message) {
        // Checking killedbyAm should keep it from showing the crash dialog if the
        // process
        ...
        if (!killedByAm) {
            ...
            if (mService != null && (noisy || info.uid == mService.mCurOomAdjUid)) {
                mService.reportUidInfoMessageLocked(TAG,
                        "Killing " + toShortString() + " (adj " + setAdj + "): " + reason, info.uid);
            }
            if (pid > 0) {
    ...
    public void resetPackageList(ProcessStatsService tracker) {
        ...
        if (baseProcessTracker != null) {
            ...
            if (N != 1) {
                ...
                pkgList.clear();
                ProcessState ps = tracker.getProcessStateLocked(info.packageName, uid, info.longVersionCode,
                        processName);
    ...
    public String getVisibleActivityName() {
        for (ActivityRecord ar : activities) {
            if (ar.nowVisible) {
                return ar.realActivity.getClassName();
            }
        }
        return null;
    }

    public IBinder getVisibleActivityToken() {
        for (ActivityRecord ar : activities) {
            if (ar.nowVisible) {
                return ar.appToken;
            }
    ...
    public DisplayInfo getDisplayInfo() {
        for (ActivityRecord ar : activities) {
            if (ar.nowVisible) {
                ActivityDisplay ad = ar.getDisplay();
                if (ad != null) {
                    DisplayInfo di = new DisplayInfo();
                    ad.mDisplay.getDisplayInfo(di);
                    return di;
                }
            }
        }
